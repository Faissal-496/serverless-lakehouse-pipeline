...
# URL-encode the RDS password so special chars ($, +, =, *, ,) don't break
# docker-compose interpolation or SQLAlchemy connection strings.
RDS_PASSWORD_ENCODED=$(python3 -c "import urllib.parse, sys; print(urllib.parse.quote(sys.argv[1], safe=''))" "$RDS_PASSWORD")

AIRFLOW_DB_CONN="postgresql+psycopg2://$RDS_USERNAME:$RDS_PASSWORD_ENCODED@$RDS_HOST:$RDS_PORT/$RDS_DBNAME$SSL_QUERY"

# Escape any $ in secret string values so docker-compose interpolation
# treats them as literal dollar signs ($ -> $$ in env-file).
escape_compose() { printf '%s' "$1" | sed 's/\$/$$/g'; }
AIRFLOW_WEBSERVER_SECRET_KEY_ESC=$(escape_compose "$AIRFLOW_WEBSERVER_SECRET_KEY")
AIRFLOW_ADMIN_PASSWORD_ESC=$(escape_compose "$AIRFLOW_ADMIN_PASSWORD")
JENKINS_ADMIN_PASSWORD_ESC=$(escape_compose "$JENKINS_ADMIN_PASSWORD")
...
AIRFLOW_WEBSERVER_SECRET_KEY=$AIRFLOW_WEBSERVER_SECRET_KEY_ESC
AIRFLOW_ADMIN_USER=$AIRFLOW_ADMIN_USER
AIRFLOW_ADMIN_PASSWORD=$AIRFLOW_ADMIN_PASSWORD_ESC
...
JENKINS_ADMIN_PASSWORD=$JENKINS_ADMIN_PASSWORD_ESC
